...
#     logger.debug(f"||{torch.multiprocessing.get_start_method()}||", end="")
...
logger = init_logger("vmamba_tiny_training", log_file="vmamba_tiny_training.log", level=logging.DEBUG)
...
def main(config, args):
    ...
    if hasattr(model, 'flops'):
            ...
            flops = model.flops()
            logger.debug(f"number of GFLOPs: {flops / 1e9}")
    else:
            logger.debug(flop_count_str(FlopCountAnalysis(model, (dataset_val[0][0][None],))))
    ...
    #         resume='')
    #     logger.debug("Using EMA with decay = %.8f" % args.model_ema_decay)
    ...
    if config.TRAIN.AUTO_RESUME:
        ...
        if resume_file:
            ...
            config.freeze()
            logger.debug(f'auto resuming from {resume_file}')
        else:
            logger.debug(f'no checkpoint found in {config.OUTPUT}, ignoring auto resume')

    if config.MODEL.RESUME:
        max_accuracy, max_accuracy_ema = load_checkpoint_ema(config, model_without_ddp, optimizer, lr_scheduler, loss_scaler, None, None)
        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.debug(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        # if model_ema is not None:
        #     acc1_ema, acc5_ema, loss_ema = validate(config, data_loader_val, model_ema.ema)
        #     logger.debug(f"Accuracy of the network ema on the {len(dataset_val)} test images: {acc1_ema:.1f}%")
    ...
    #     logger.debug(f"throughput mode ==============================")
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, None)
        if epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1):
            save_checkpoint_ema(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler, None, None, max_accuracy_ema)
        ...
        # if model_ema is not None:
        #     acc1_ema, acc5_ema, loss_ema = validate(config, data_loader_val, model_ema.ema)
        #     logger.debug(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1_ema:.1f}%")
        #     max_accuracy_ema = max(max_accuracy_ema, acc1_ema)
        #     logger.debug(f'Max accuracy ema: {max_accuracy_ema:.2f}%')
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, model_time_warmup=50):
    ...
    logger.debug(f"EPOCH {epoch} training takes {datetime.timedelta(seconds=int(epoch_time))}")
...
def validate(config, data_loader, model):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        logger.debug(
                ...
                f'Mem {memory_used:.0f}MB')
    logger.debug(f' * Acc@1 {acc1_meter.avg:.3f} Acc@5 {acc5_meter.avg:.3f}')
...
def throughput(data_loader, model, logger):
    ...
    for idx, (images, _) in enumerate(data_loader):
        ...
        for i in range(50):
            model(images)
        torch.cuda.synchronize()
        logger.debug(f"throughput averaged with 30 times")
        tic1 = time.time()
        for i in range(30):
            model(images)
        torch.cuda.synchronize()
        tic2 = time.time()
        logger.debug(f"batch_size {batch_size} throughput {30 * batch_size / (tic2 - tic1)}")
        return
...
if __name__ == '__main__':
    ...
    if config.AMP_OPT_LEVEL:
        logger.debug("[warning] Apex amp has been deprecated, please use pytorch amp instead!")

    # if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
    #     rank = int(os.environ["RANK"])
    #     world_size = int(os.environ['WORLD_SIZE'])
    #     logger.debug(f"RANK and WORLD_SIZE in environ: {rank}/{world_size}")
    # else:
    #     rank = -1
    #     world_size = -1
    ...
    #         f.write(config.dump())
    #     logger.debug(f"Full config saved to {path}")

    # logger.debug config
    # logger.debug(config.dump())
    # logger.debug(json.dumps(vars(args)))
