        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            if token_merge:
                ...
                prod = dts * As.view(1, As.shape[0], 1)

                cum = prod.cumsum(dim=-1)

                cum = torch.clamp(cum, -0.0000005, 0.0000005)

                m = cum.max(dim=1).values

                cum = cum - m.unsqueeze(1)

                exp_pos = torch.exp(cum)
                exp_neg = torch.exp(-cum)

                exp_pos = exp_pos.unsqueeze(1)   # [B, 1, N, L]
                exp_neg = exp_neg.unsqueeze(1)

                V = Cs * exp_pos
                U = Bs * exp_neg

                alpha = torch.einsum('bcnl, bcnm -> bc lm', V, U)

                alpha = alpha.mean(dim=1)

                result = alpha.sum(dim=2)

                result_norm = F.normalize(result, p=2, dim=1)

                cos_sim_alpha = torch.bmm(result_norm.unsqueeze(2), result_norm.unsqueeze(1))

                cos_sim_final = torch.softmax(cos_sim_alpha, dim=-1)

                ys = torch.bmm(ys, cos_sim_final)
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        # if self.counter == 1:
        #     # self.token_transformer = # make token transformer
        #     print(x.shape)
